...
export function Hero({ game, onDownload, onSelect, isDownloading, checkingAvailability, isInstalled }: HeroProps) {
    ...
    return (
      <div className="w-full h-[500px] rounded-3xl bg-gradient-to-br from-primary/10 via-primary/5 to-background flex items-center justify-center border border-muted/20 relative overflow-hidden group">
        ...
        <div className="text-center z-10 space-y-4 max-w-lg p-6">
          <h2 className="text-4xl font-bold tracking-tight">Discover Your Next Game</h2>
          <p className="text-muted-foreground text-lg">
            Search for any Steam game to check availability and download directly.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="w-full h-[500px] rounded-3xl relative overflow-hidden group shadow-lg bg-muted">
      ...
      <div className="absolute bottom-0 left-0 p-8 space-y-4 max-w-2xl">
        <div className="space-y-2">
            ...
            <h2 className="text-5xl font-bold tracking-tight text-foreground drop-shadow-md">
            {game.name}
            </h2>
            <p className="text-muted-foreground font-mono">App ID: {game.id}</p>
        </div>
        ...
        <div className="flex gap-4 pt-4">
          {isInstalled ? (
             <Button 
                size="lg" 
                className="rounded-full px-8 gap-2 font-semibold shadow-lg bg-green-600 hover:bg-green-700 text-white cursor-default"
                disabled
              >
             ...
             <Button 
                size="lg" 
                className="rounded-full px-8 gap-2 font-semibold shadow-lg shadow-primary/20"
                onClick={onDownload}
